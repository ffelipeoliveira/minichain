...
public class Print {
    ...
    private static final int MAX_STR_LENGTH = 23; // A block has 27 in length, including the two borders and two spaces
                                                  // at beginning and end
    private static final int MAX_INT_LENGTH = 10; // The label that come before numbers are 13 characters long

    // The default number of available space for an number to be printed is 10 since
    // they need an extra label (id, hash, nonce which is not implemented yet, and
    // the previous hash)
    private static void printSpace(int number, int maxIntLength) {
        for (int i = 0; i < (maxIntLength - App.getNumeralQtt(number)); i++) {
            System.out.print(" ");
        }

        // Negative numbers take up a extra character -
        if (number >= 0)
            System.out.print(" ");
        System.out.print(" ║   ");
    }

    // The default number of available space for an information to be printed is 23
    // characters, since they don't need an extra label
    private static void printSpace(String str, int maxStrLength) {
    ...
    // Basic print of a full block information
    public static void PrintFullBlock(Block block) {
        System.out.println(
                "Showing full block information\n╔════════════════════════════════════════════════════════════════════════╗ ");
    ...
    // Will print an Array of blocks horizontally line by line
    public static void PrintBlockchain(Blockchain blockChain) {
        ...
        if (blockQtt > MAX_BLOCKS_PER_ROW) {
            // This division will round up to the nearest int, since a full row is needed to
            // print even if it's just only one block
            rows = (blockQtt + MAX_BLOCKS_PER_ROW - 1) / MAX_BLOCKS_PER_ROW;
        }
        ...
        for (int i = 0; i < rows; i++) {
            int maxDataLength = 1;
            // To erase the previous blocks a new object is created, the previous object,
            // now untracked, will be discarded by the JVM
            row = new Block[MAX_BLOCKS_PER_ROW];
            for (int j = 0; (j < MAX_BLOCKS_PER_ROW && currentBlock != null); j++) {
                row[j] = currentBlock;
                if (currentBlock.getData().length > maxDataLength)
                    maxDataLength = currentBlock.getData().length;
            ...
            for (int j = 0; (j < MAX_BLOCKS_PER_ROW && row[j] != null); j++) {
                System.out.print("║ THIS HASH : "
                        + row[j].getHash().substring(0, Math.min(row[j].getHash().length(), 8)) + "...");
            ...
            for (int j = 0; (j < MAX_BLOCKS_PER_ROW && row[j] != null); j++) {
                if (row[j].getPrevHash() != null) {
                    System.out.print("║ PREV HASH : "
                            + row[j].getPrevHash().substring(0, Math.min(row[j].getHash().length(), 8)) + "...");
            ...
            for (int j = 0; (j < MAX_BLOCKS_PER_ROW && row[j] != null); j++) {
                ...
                if (row[j].getPrevious() != null) {
                    if (App.validateLink(row[j], row[j].getPrevious()))
                        System.out.print("===");
                    else
                        System.out.print("=X=");
                }
                ...
                for (int k = 0; (k < MAX_BLOCKS_PER_ROW && row[k] != null); k++) { // K is the index of the block
                    if (row[k].getData().length > j) {
                        ...
                        if (strLength >= MAX_STR_LENGTH)
                            System.out.print("║ " + row[k].getData()[j].toString().substring(0,
                                    Math.min(strLength, MAX_STR_LENGTH - 3)) + "... ║   ");
                        else {
